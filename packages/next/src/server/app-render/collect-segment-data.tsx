...
export async function collectSegmentData(
  ...
  try {
    const prerenderResult = await prerender(
      // NavigationFlightResponse is not a valid return type for a React
...
async function PrefetchTreeData({
  ...
  shellStageRelease: Promise<boolean>
}): Promise<NavigationFlightResponse | null> {
  ...
  // ordinary NavigationFlightResponse carrying only a buildId and a
  // structure-only tree.
...
async function renderSegmentPrefetch(
  ...
  // `a` (see NavigationFlightResponse['a']) is resolved below, once the
  ...
  // Wrap in the response envelope — an ordinary NavigationFlightResponse.
  const payload: NavigationFlightResponse = {
    t: transportData,
