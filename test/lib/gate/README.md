...
## Covering both states of an experiment: the variant shard

CI runs the suites twice — a plain run and a *variant* run, marked by
`__NEXT_TEST_VARIANT=true` (an alias for the `--experimental` /
`__NEXT_CACHE_COMPONENTS` shard for now; see `scripts/run-jest.sh`). A fixture
can key an experimental flag on the marker instead of pinning it — enabled by
default, disabled in the variant run:
...
a local run with no special env — exercises the feature, while the variant run
...
To reproduce the disabled (variant) state locally, set the marker the same way
CI does:

```sh
__NEXT_TEST_VARIANT=true NEXT_SKIP_ISOLATE=1 pnpm test-start-webpack test/e2e/app-dir/use-offline/use-offline.test.ts
```
